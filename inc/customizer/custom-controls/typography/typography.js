 ...
 */
 ...
 ( function( $ ) {
	...
	InspiroTypography = {
		...
		_initFonts: function()
		{
			$( '.customize-control-inspiro-font-family select' ).each( function(e) {

				if( 'undefined' != typeof inspiroCustomControl.customizer ) {
					var fonts = inspiroCustomControl.customizer.settings.google_fonts;
			...
			$( '.customize-control-inspiro-font-family select' ).each( InspiroTypography._initFont );
			// Added select2 for all font family & font variant.
			$('.customize-control-inspiro-font-family select, .customize-control-inspiro-font-variant select').selectWoo();

			$('.customize-control-inspiro-font-variant select').on('select2:unselecting', function (e) {
		...
		_setFontVarianttOptions: function( init )
		{
				...
				variantSavedField   = api.control( variants ).container.find( '.inspiro-font-variant-hidden-value' ),
				weightValue        = '',
				weightOptions       = '',
	...
	$( function() { InspiroTypography.init(); } );
